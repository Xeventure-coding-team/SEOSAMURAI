...
const SphereAnimation = () => {
  ...
  useEffect(() => {
    ...
    // Initialize stroke dashoffset animation
    spherePathEls.forEach((path, i) => {
      ...
      // Smoother staggered intro animation
      const delay = (pathLength - i) * 50; // Reduced delay for faster start
      const duration = 2500 + (pathLength - i) * 150; // Shorter duration for smoother flow
      
      setTimeout(() => {
        pathElement.style.transition = `stroke-dashoffset ${duration}ms cubic-bezier(0.4, 0.0, 0.2, 1)`; // Smoother easing
        pathElement.style.strokeDashoffset = "0";
      }, delay);
    });

    // Combined animation loop for paths and gradient
    let time = 0;
    const animate = () => {
      time += 0.016; // Approximate 60fps (1000ms / 60 ≈ 16ms)

      // Animate paths
      spherePathEls.forEach((path, i) => {
        const pathElement = path;
        const percent = (1 - Math.sin(i * 0.3 + time)) / 2; // Slightly slower phase shift
        
        // Animate stroke color
        const r = Math.floor(232 + percent * 20);
        const g = Math.floor(121 + percent * 35);
        const b = Math.floor(249);
        const opacity = 0.4 + percent * 0.6; // Slightly adjusted opacity range
        
        pathElement.style.stroke = `rgba(${r}, ${g}, ${b}, ${opacity})`;
        
        // Animate position with smoother translation
        const tx = 1.5 + percent * -5; // Reduced translation range
        const ty = 1.5 + percent * -5;
        pathElement.style.transform = `translate(${tx}px, ${ty}px)`;
        pathElement.style.transition = `transform 16ms linear, stroke 16ms linear`; // Smooth transitions
      });

      // Animate gradient
      const gradient = sphereEl.querySelector("#sphereGradient");
      if (gradient) {
        const gradientTime = time * 0.005; // Slower gradient movement
        const x = 5 + Math.sin(gradientTime) * 15; // Reduced range for smoother effect
        const y = 15 + Math.cos(gradientTime) * 50;
        ...
        >
          <defs>
            <linearGradient
              ...
              y2="15%"
            >
              <stop stopColor="rgba(232,121,249,0.1)" offset="0%" />
              <stop stopColor="rgba(168,85,247,0.05)" offset="50%" />
              <stop stopColor="rgba(139,92,246,0.02)" offset="100%" />
            </linearGradient>
